...
public interface LogEventRepository extends JpaRepository<LogEventEntity, String> {
    ...
    @Query("SELECT l FROM LogEventEntity l WHERE l.timestamp >= :since ORDER BY l.timestamp DESC")
    List<LogEventEntity> findRecentLogs(@Param("since") LocalDateTime since);
